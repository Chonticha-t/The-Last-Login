...
const AuthView: React.FC<AuthViewProps> = ({ status, onComplete, onRequestHint }) => {
  ...
  const [rooms, setRooms] = useState<SuspectRoom[]>([
    ...
    },
    {
      ...
      icon: "biotech",
      description: "ระบบความปลอดภัยที่ต้องใช้ 'ตรรกะ' โดยการนำข้อมูลจากสองแหล่งที่ดูไม่เกี่ยวข้องกัน มาคำนวณหาคำตอบ",

      // โจทย์ภาษาคน: ต้องหาตัวแปร 2 ตัวมาลบกัน
      challenge: "Password Logic: [รหัสสารเคมีตั้งต้น (Periodic Table No.)] - [จำนวนแก้วกาแฟที่วางทิ้งไว้]",

      // คำตอบ: สารหนู (Arsenic = 33) - 5 แก้ว = 28
      answer: "28",

      // คำใบ้: ชี้เป้าให้มองหาสิ่งของในห้อง และความรู้รอบตัว
      hint: "ลองเปิดไฟล์ บันทึกการทดลอง_As.PDF ดู อะไรคือสารตั้งต้น? มีกาแฟกี่แก้วที่ถูกทิ้งไว้?",

      evidence: ["เอกสารโครงงานวิจัย", "สนิทกับนายคำปัน"],

      // พฤติกรรม: ปรับให้ดูหวาดระแวง (Paranoid) และมีพิรุธเรื่องการทำลายหลักฐาน
      behavior: "มีอาการตื่นตระหนกทุกครั้งที่มีเสียงเรียกเข้า มักจะสวมถุงมือยางตลอดเวลาแม้ไม่ได้ทำการทดลอง",
  ...
  const evidenceFiles = [
    ...
    },
   {
      ...
      status: 'ENCRYPTED',
      content: "สรุปผลการทดลองโครงการ Toxic:\nสารตั้งต้นหลัก: Arsenic (As)\nเลขอะตอม: ? \n\nบันทึกส่วนตัว: วันนี้เครียดมากจนต้องอัดกาแฟ ผมเพิ่งแกะกล่องแคปซูลกาแฟกล่องใหม่ (ขนาดครึ่งโหล) มาชงดื่ม... ตะกี้ชะโงกดูในกล่อง เหลือแคปซูลอันสุดท้ายอันเดียวซะแล้ว",
      icon: 'description'
    },
    {
                                ...
                                onClick={(e_ev) => {
                                  ...
                                  if (ev === "บทสวดดั้งเดิม") {
                                    ...
                                    switch (ev) {
                                      ...
                                      case "บันทึกเวรยาม":
                                        specificContent = `[สมุดบันทึกรายงานเหตุการณ์ประจำวัน]\n
วันที่ 5
จุดตรวจ: ป่าหลัง ม. (โซนริมแม่น้ำ)
เวลา: 03:15 น.
เหตุการณ์: พบชาย 2 นาย แบกวัตถุลักษณะยาวห่อผ้าขาวเปื้อนคราบสีเข้ม มุ่งหน้าทางตลิ่ง
การปฏิบัติ: รีบถอนกำลังออกจากจุดเกิดเหตุทันที
หมายเหตุ: ได้รับ "คำสั่งด่วนพิเศษ" ให้รีบไปตรวจสอบเหตุที่โรงอาหารกลาง (ไม่พบเหตุ) จึงจำเป็นต้องละเว้นการตรวจสอบจุดนี้\n
-------------------------\n
วันที่ 12
จุดตรวจ: สระสามแสน
เวลา: 03:30 น.
เหตุการณ์: รถกระบะสีดำจอดเทียบท่าจุดอับแสง ได้ยินเสียงวัตถุหนักตกน้ำ
การปฏิบัติ: -
หมายเหตุ: กล้องวงจรปิดแจ้งเสีย ตั้งแต่ 23:30 น. ไม่สามารถบันทึกภาพได้ แก้ไขไม่ได้จนถึงเช้า\n
-------------------------\n
วันที่ 20
จุดตรวจ: อุโมงค์ต้นไม้
เวลา: 03:20 น.
เหตุการณ์: ไม่พบสิ่งผิดปกติ
การปฏิบัติ: ปิดกั้นเส้นทางชั่วคราว อ้างซ่อมบำรุงผิวจราจร
หมายเหตุ: -`;
                                        break;
                                      case "ภาพจากกล้องวงจรปิดที่หายไป":
                                        specificContent = `[RECOVERED FOOTAGE - CCTV 04]\n
Timestamp: 03:10 น. (ก่อนเกิดเหตุเล็กน้อย)\n
ภาพเหตุการณ์: รปภ. เดินวนไปวนมาหน้ากล้องด้วยท่าทางลุกลี้ลุกลน กัดเล็บและยกนาฬิกาขึ้นดูถี่ยิบ เหงื่อท่วมตัวทั้งที่อากาศเย็น... สายตากรอกไปมาเหมือนหวาดระแวงว่ามีใครจ้องมองอยู่\n
\n
จู่ๆ เขาหันขวับมาจ้องที่เลนส์กล้องด้วยสีหน้าตื่นตระหนก ก่อนจะเอื้อมมือที่สั่นเทาเข้ามาตะปบที่ตัวกล้อง (ภาพสั่นไหวรุนแรง) เหมือนพยายามจะขยับมุมหนีหรือดึงสายไฟด้วยความสติแตก...\n
\n
(เสียงพึมพำสั่นเครือสุดท้ายก่อนภาพตัด) "...ไปสิ... ไปให้พ้นๆ... ทำไมต้องมาให้เห็นตอนนี้... บอกแล้วไงว่าอย่ามายืนตรงนี้... อย่าจ้องผมแบบนั้น!!"`;
                                        break;
                                      case "เอกสารโครงงานวิจัย": specificContent = "พบบางหน้าหายไป"; break;
                                      case "สนิทกับนายคำปัน":
                                        specificContent = `[RECOVERED CHAT_LOG: PRIVATE MESSAGE]
------------------------------------------------
(Time: เช้าวันรับศพ)

08:30 [Dr.Manas]: ปัน... ข้าเคลียร์ทางนิติเวชให้แล้วนะ เอ็งไม่ต้องเข้ามาทำเรื่องที่คณะให้วุ่นวาย
08:31 [Dr.Manas]: ข้าเซ็นรับรองให้เองว่าเป็น 'อุบัติเหตุ' จะได้เอาร่างเจ้าตุ๊ออกไปประกอบพิธีได้ไวๆ
08:35 [คำปัน]: ขอบใจมากนะมนัส... ถ้าไม่ได้เอ็ง ข้าคงยืนรอไม่ไหว ขามันไม่มีแรงเลย
08:36 [Dr.Manas]: ไม่ต้องพูดงั้น... เราเพื่อนกันมาครึ่งค่อนชีวิต สมัยเรียนเอ็งช่วยข้าไว้เยอะ เรื่องแค่นี้ข้าทำให้ได้
08:37 [Dr.Manas]: ส่วนเรื่องค่าโลง ค่ารถ เดี๋ยวข้าจัดการให้หมด เอ็งไปรอรับหลานที่วัดเถอะ
08:40 [คำปัน]: ข้าไม่ลืมบุญคุณเอ็งเลยเพื่อน...
------------------------------------------------`;
                                        break;
