...
const TriVariablePuzzle: React.FC<TriVariablePuzzleProps> = ({ onUnlock }) => {
  const [activeTab, setActiveTab] = useState(0); // 0=North, 1=East, 2=South, 3=Final
  
  // ตัวแปรที่ผู้เล่นต้องหา (เฉลยจริง)
  // NORTH: Depth(5) * Density(3) = 15
  // EAST: (P(15) - Angle(5)) / 2 = 5
  ...
  // Check Final Answer
  const handleSubmit = () => {
    if (finalInput === "150509") {
      alert("ACCESS GRANTED: CHAIN COMPLETED");
      onUnlock();
    } else {
      setLogs(prev => [`> ERROR: INVALID HASH. RE-CALCULATE CHAIN.`, ...prev]);
    }
  };
          ...
          <div className="w-full h-full flex flex-col p-6 relative">
            ...
            <div className="flex gap-8 h-full">
              ...
              <div className="w-1/2 bg-slate-800 relative h-[300px] border-r-4 border-dashed border-gray-600">
                ...
                <div className="absolute top-[200px] w-full h-[50px] bg-red-900/50 flex items-center justify-center text-white font-bold">TARGET (CORPSE)</div>
                <div className="absolute top-[250px] w-full h-[50px] bg-stone-900"></div>
        ...
        {activeTab === 3 && (
          <div className="w-full h-full flex flex-col items-center justify-center p-6 bg-cyan-900/10">
             ...
             <div className="w-full max-w-sm">
               <label className="text-xs text-gray-400 mb-1 block">ENTER CHAIN HASH (PRT)</label>
               <input 
                 type="text" 
